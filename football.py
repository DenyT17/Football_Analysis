...
Ronaldo=scorers_belong_year("Cristiano Ronaldo",2015,appearances_data,players_data)
Messi=scorers_belong_year("Lionel Messi",2015,appearances_data,players_data)
Bale=scorers_belong_year("Gareth Bale",2015,appearances_data,players_data)
Neymar=scorers_belong_year("Neymar",2015,appearances_data,players_data)
two_players=pd.concat([Ronaldo,Messi,Bale,Neymar]).sort_values(by=['date']).reset_index(drop=True)
fig,ax=plt.subplots(figsize=(15,10))
sns.lineplot(data=two_players,x='date',y='Goal Sum',hue='Player name',linewidth=4)
plt.title("Players goals during the season",fontsize=15)
plt.show()